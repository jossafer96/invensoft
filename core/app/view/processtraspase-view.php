...
//////////////////////////////////
		...
		foreach($cart as  $c){
			...
			$name_stock=StockData::getById($_SESSION["stock_id"]);
			$description_operation_start= 'Traspaso de equipo desde almacen '.$name_stock->name;
			...
			$op = new OperationData();
			$name_stock=StockData::getById($_SESSION["stock_id"]);
			$description_operation_end= 'Traspaso de equipo desde otro almacen';
			$op->price_in = $product->price_in;
			$op->price_out = $product->price_out;
			$op->product_id = $c["product_id"] ;
			...
			$op->sell_id=$s[1];
			$op->q= $c["q"];
			$op->description_operation=$description_operation_end;
			$op->user=$user_id;
			$add = $op->add();
